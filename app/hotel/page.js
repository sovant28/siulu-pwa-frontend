    ...
    <div className="flex flex-col w-full min-h-[100dvh] bg-[#F6F7F9] font-sans pb-[calc(env(safe-area-inset-bottom)+76px)] relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md pt-[calc(env(safe-area-inset-top)+8px)] pb-3 flex flex-col gap-3 border-b border-slate-200">
        ...
        <div className="flex items-center gap-3 px-5">
          ...
          <h1 className="text-[17px] font-black text-slate-800 select-none">Hotel & Akomodasi</h1>
        </div>
      ...
      {!loading && (
        <div className="px-5 mt-3">
          <p className="text-[11px] font-semibold text-slate-400">{filteredHotels.length} akomodasi ditemukan</p>
        </div>
      )}

      {/* ── HOTEL LIST (BORDERLESS CARD VIEW) ── */}
      <div className="px-5 mt-4 space-y-9 pb-4">
        {loading ? (
          ...
          <>
            {filteredHotels.slice(0, visibleCount).map((dest) => {
          ...
          <div className="text-center py-16 px-6">
            ...
            <p className="text-sm text-slate-500 font-bold">Tidak ada hotel atau penginapan yang cocok.</p>
